...
ParametersLiteral make_parameters(const ResNet50GpuConfig &config)
{
    config.validate();
    ...
    const LinearMatrixGroup &slot_to_coeff)
{
    ...
    // Report which ResNet50 power-of-two rotations are already covered by the
...
{
public:
    Impl(const ResNet50GpuConfig &config, int device_id)
        : device_id(device_id),
          rmm_scope(device_id),
...
GpuCkksRuntime::GpuCkksRuntime(const ResNet50GpuConfig &config, int device_id)
...
GpuCkksRuntime::DeviceCiphertext
GpuCkksRuntime::multiply_plain_rescale_cached_lazy(
    const DeviceCiphertext &source,
    const std::string &cache_key,
    const std::function<std::vector<double>()> &plain_builder) const
{
    if (!impl_->full_device_cache)
    {
        return multiply_plain_rescale(source, plain_builder());
    }
    const auto context_data =
        impl_->context.crt_context()->get_context_data(source.meta.parms_id);
    if (!context_data || context_data->coeff_modulus().empty())
    {
        throw std::invalid_argument(
            "multiply_plain_rescale_cached_lazy received an unknown modulus level");
    }
    const auto full_key = cache_key + ":q=" +
                          std::to_string(source.meta.q_count);
    auto found = impl_->plaintext_cache.find(full_key);
    if (found == impl_->plaintext_cache.end())
    {
        const auto plain_slots = plain_builder();
        const double plain_scale = static_cast<double>(
            context_data->coeff_modulus().back().value());
        Plaintext host_plaintext;
        impl_->encoder.encode(
    ...
    double value) const
{
    ...
    // A CKKS plaintext containing one real scalar in every slot is a constant
    // polynomial. When its scaled integer fits one GPU word, multiply the
    // ciphertext directly by that integer instead of allocating and caching a
    // full N*Q NTT plaintext. This is essential for ImageNet stems with
    // thousands of distinct scalar weights and is algebraically identical to
    // encoding the scalar at plain_scale.
    const long double scaled_abs =
        std::abs(static_cast<long double>(value)) * plain_scale;
    if (scaled_abs <=
        static_cast<long double>(std::numeric_limits<gpu::GpuWord>::max()))
    {
        const auto scalar = static_cast<std::uint64_t>(
            std::floor(scaled_abs + 0.5L));
        DeviceCiphertext unsigned_product;
        impl_->gpu_evaluator.multiply_scalar(source, scalar, unsigned_product);
        unsigned_product.meta.scale = source.meta.scale * plain_scale;
        if (value >= 0.0)
        {
            return rescale(unsigned_product, 1);
        }
        DeviceCiphertext negative_product;
        impl_->gpu_evaluator.negate(unsigned_product, negative_product);
        return rescale(negative_product, 1);
    }
...
void GpuCkksRuntime::clear_plaintext_cache()
{
    impl_->plaintext_cache.clear();
}

void GpuCkksRuntime::clear_ciphertext_cache()
{
    impl_->ciphertext_cache.clear();
}
...
void GpuCkksRuntime::initialize_resnet50_evaluation_keys()
{
