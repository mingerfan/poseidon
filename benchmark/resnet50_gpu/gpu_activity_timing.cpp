                         ...
                         record->kind == CUPTI_ACTIVITY_KIND_KERNEL)
                {
                    ...
                    active->activities.push_back(
                        {activity.start, activity.end, 0,
                         ...
                         activity.name ? activity.name : "<unnamed>"});
                }
...
GpuActivityTimingResult GpuActivityTiming::finish()
{
    ...
    for (const auto &[stage, nanoseconds] : summary.stage_kernel_ns)
        result.stages[names.at(stage)].kernel_ms =
            static_cast<double>(nanoseconds) / 1.0e6;
    for (const auto &[stage, nanoseconds] : summary.stage_device_copy_ns)
        result.stages[names.at(stage)].device_copy_ms =
            static_cast<double>(nanoseconds) / 1.0e6;
    for (const auto &[stage, nanoseconds] : summary.stage_memset_ns)
        result.stages[names.at(stage)].memset_ms =
            static_cast<double>(nanoseconds) / 1.0e6;

    std::map<std::string,
             std::map<std::string, std::vector<GpuTimeInterval>>>
        kernel_intervals;
    for (const auto &activity : impl_->activities)
    {
        if (activity.kind != 0)
        ...
        if (correlation == impl_->correlations.end() ||
            !names.count(correlation->second))
            throw std::runtime_error(
                "GPU kernel activity has no measured-stage attribution");
        kernel_intervals[names.at(correlation->second)][activity.name]
            .emplace_back(activity.start, activity.end);
    }
    for (auto &[stage, kernels] : kernel_intervals)
    {
        for (auto &[name, intervals] : kernels)
        {
            result.stage_kernels_ms[stage][name] =
                static_cast<double>(gpu_interval_union_ns(
                    std::move(intervals))) / 1.0e6;
        }
    }
    return result;
}
...
void GpuActivityTimingResult::print() const
{
    ...
    for (const auto &[label, stage] : stages)
    {
        std::cout << "GPU_ACTIVITY_STAGE label=" << label
                  << " gpu_ms=" << stage.gpu_ms
                  << " kernel_ms=" << stage.kernel_ms
                  << " device_copy_ms=" << stage.device_copy_ms
                  << " memset_ms=" << stage.memset_ms << '\n';
        const auto found = stage_kernels_ms.find(label);
        if (found == stage_kernels_ms.end())
            continue;
        ...
        const std::size_t limit = std::min<std::size_t>(12, kernels.size());
        for (std::size_t index = 0; index < limit; ++index)
        {
            std::cout << "GPU_ACTIVITY_KERNEL stage=" << label
                      << " kernel=" << kernels[index].first
                      << " kernel_ms=" << kernels[index].second << '\n';
        }
    }
