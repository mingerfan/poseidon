...
public:
    ...
    explicit GpuCkksRuntime(const ResNet50GpuConfig &config, int device_id = 0);
    ~GpuCkksRuntime();
    ...
    // Builds/encodes the plaintext only on a cache miss. Prepared GPU-only
    // replays therefore avoid reconstructing large SIMD vectors on the CPU.
    DeviceCiphertext multiply_plain_rescale_cached_lazy(
        const DeviceCiphertext &source,
    ...
    // Release expanded model plaintexts between layer-wise preparation
    // windows while retaining keys, bootstrap matrices and live activations.
    void clear_plaintext_cache();
    void clear_ciphertext_cache();
    ...
    void initialize_resnet50_evaluation_keys();
