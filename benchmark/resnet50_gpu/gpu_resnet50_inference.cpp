...
constexpr double kBoundary = 120.0;
    ...
    const std::vector<double> &weight)
{
    ...
    {
        throw std::invalid_argument("ResNet50 batch-normalization shape mismatch");
    }
...
void log_tensor_state(const std::string &label, const GpuMultiplexedTensor &tensor)
{
    ...
    {
        std::cout << "[GPU ResNet50] " << label << " empty\n";
        return;
    }
...
auto timed(const std::string &label, Function &&function)
{
    ...
    std::cout << "[GPU ResNet50] " << label << " elapsed_ms=" << elapsed << '\n';
    return result;
}
    ...
    const GpuCkksRuntime &runtime)
{
    ...
    for (std::size_t pack = 0; pack < input.packs.size(); ++pack)
    {
        output.packs[pack] = polynomial_relu(input.packs[pack], runtime);
        std::cout << "[GPU ResNet50] ReLU pack " << pack + 1 << '/'
                  << input.packs.size() << '\n';
    }
    return output;
}
    ...
    const std::vector<double> *expected_bootstrap = nullptr)
{
    ...
    if (expected_bootstrap)
    {
        verify_values(
            ...
            decrypt_multiplexed_chw(refreshed, runtime), 1.0e-1);
    }
...
std::vector<double> plain_stem_relu_pool(std::vector<double> stem)
{
    ...
    {
        value = polynomial_relu_reference(value);
    }
    std::vector<double> output(static_cast<std::size_t>(64 * output_h * output_w));
...
PlainTensor plain_add(PlainTensor left, const PlainTensor &right)
{
    ...
    {
        throw std::invalid_argument("plain residual shape mismatch");
    }
    ...
    double tolerance)
{
    ...
    {
        maximum_error = std::max(maximum_error, std::abs(expected[index] - actual[index]));
    }
    std::cout << "[GPU ResNet50] " << label << " max_abs_error="
              << maximum_error << '\n';
...
PlainTensor plain_resnet50_features(
    const std::vector<double> &image,
    ...
    const ResNet50Weights &weights)
{
    ...
    for (const auto &spec : topology.blocks)
    {
        ...
        auto branch = plain_conv_bn(
            tensor, spec.bottleneck_channels, 1, 1,
        ...
        branch = plain_conv_bn(
            branch, spec.bottleneck_channels, spec.stride, 3,
            weights.conv_weight[conv_index], affine);
        ++conv_index;
        ++bn_index;
        plain_relu_inplace(branch);
        ...
        affine = folded_batch_norm(
            ...
            weights.bn_running_var[bn_index], weights.bn_weight[bn_index]);
        branch = plain_conv_bn(
            branch, spec.output_channels, 1, 1,
            weights.conv_weight[conv_index], affine);
        ++conv_index;
        ++bn_index;
        ...
        if (spec.projection)
        {
            const auto downsample = static_cast<std::size_t>(spec.stage - 1);
            affine = folded_batch_norm(
...
GpuResNet50Result run_gpu_resnet50_impl(
    std::size_t image_id,
    const ResNet50Topology &topology,
    const ResNet50Weights &weights,
    ...
    bool measure_staged_gpu_only)
{
    ...
    {
        throw std::invalid_argument("ResNet50 max_blocks exceeds the topology");
    }
    GpuResNet50Result result;
    ...
    const auto finish_gpu_only = [&]() {
        ...
        std::cout << "[GPU ResNet50] gpu_only_preloaded_elapsed_seconds="
    ...
    auto tensor = stage_timer.run("stem.conv1_bn1", [&]() {
        return encrypted_stem_conv2d_bn(
    ...
    if (validate_intermediates)
    {
        auto pooled_reference = plain_stem_relu_pool(std::move(plain_stem));
        plain_tensor = PlainTensor{56, 56, 64, std::move(pooled_reference)};
        verify_values(
            "stem.relu_avgpool", plain_tensor->values,
            decrypt_multiplexed_chw(tensor, runtime), 2.0e-3);
    }

    for (std::size_t block_index = 0; block_index < max_blocks; ++block_index)
    {
        const auto &spec = topology.blocks[block_index];
        const std::string prefix = bottleneck_name(spec);
        ...
        if (plain_tensor)
        {
            ...
            plain_branch = plain_conv_bn(
                *plain_tensor, spec.bottleneck_channels, 1, 1,
                weights.conv_weight[conv_index], affine);
        }
        ...
        if (plain_branch)
        {
            verify_values(
                ...
                decrypt_multiplexed_chw(branch, runtime), 2.0e-3);
        }
        ++conv_index;
        ++bn_index;
        ...
        if (plain_branch)
        {
            ...
            verify_values(
                ...
                decrypt_multiplexed_chw(branch, runtime), 1.0e-1);
            const auto affine = folded_batch_norm(
                weights.bn_bias[bn_index], weights.bn_running_mean[bn_index],
                weights.bn_running_var[bn_index], weights.bn_weight[bn_index]);
            *plain_branch = plain_conv_bn(
                *plain_branch, spec.bottleneck_channels, spec.stride, 3,
                weights.conv_weight[conv_index], affine);
        }
        ...
        if (plain_branch)
        {
            verify_values(
                ...
                decrypt_multiplexed_chw(branch, runtime), 2.0e-2);
        }
        ++conv_index;
        ++bn_index;
        branch = bootstrap_relu(
            prefix + ".act2", branch, runtime, stage_timer,
            plain_branch ? &plain_branch->values : nullptr);
        ...
        if (plain_branch)
        {
            plain_relu_inplace(*plain_branch);
            verify_values(
                prefix + ".act2", plain_branch->values,
                decrypt_multiplexed_chw(branch, runtime), 2.0e-1);
            const auto affine = folded_batch_norm(
                weights.bn_bias[bn_index], weights.bn_running_mean[bn_index],
                weights.bn_running_var[bn_index], weights.bn_weight[bn_index]);
            *plain_branch = plain_conv_bn(
                *plain_branch, spec.output_channels, 1, 1,
                weights.conv_weight[conv_index], affine);
        }

        branch = stage_timer.run(prefix + ".conv3_bn3", [&]() {
            return apply_conv_bn(
                branch, spec.output_channels, 1, 1,
                weights.conv_weight[conv_index], weights.bn_bias[bn_index],
                weights.bn_running_mean[bn_index], weights.bn_running_var[bn_index],
        ...
        {
            verify_values(
                prefix + ".conv3_bn3", plain_branch->values,
                decrypt_multiplexed_chw(branch, runtime), 5.0e-1);
        }
        ++conv_index;
        ++bn_index;
        ...
        if (spec.projection)
        {
            ...
            if (plain_shortcut)
            {
                verify_values(
                    ...
                    decrypt_multiplexed_chw(shortcut, runtime), 2.0e-3);
            }
        }
        tensor = stage_timer.run(prefix + ".add", [&]() {
            return residual_add(branch, shortcut, runtime);
        });
        if (plain_branch && plain_shortcut)
        {
            plain_tensor = plain_add(
                std::move(*plain_branch), *plain_shortcut);
            verify_values(
                prefix + ".add", plain_tensor->values,
                decrypt_multiplexed_chw(tensor, runtime), 5.0e-1);
        }
        tensor = bootstrap_relu(
            prefix + ".act3", tensor, runtime, stage_timer,
            plain_tensor ? &plain_tensor->values : nullptr);
        if (plain_tensor)
        {
            plain_relu_inplace(*plain_tensor);
            verify_values(
                prefix + ".act3", plain_tensor->values,
                decrypt_multiplexed_chw(tensor, runtime), 1.0);
        }
    ...
    if (max_blocks != topology.blocks.size())
    {
        ...
        if (measure_staged_gpu_only)
        {
            ...
            std::cout << "[GPU ResNet50] staged_gpu_only_elapsed_seconds="
                      << result.gpu_only_elapsed_seconds << '\n';
        }
        if (plain_tensor && max_blocks == 1)
        {
            verify_values(
                "layer1.0.output", plain_tensor->values,
                decrypt_multiplexed_chw(tensor, runtime), 5.0e-2);
        }
        std::cout << "[GPU ResNet50] staged run stopped after " << max_blocks
                  << " bottleneck block(s)\n";
        return result;
    }

    auto features = stage_timer.run("head.global_avgpool", [&]() {
        return global_average_pool(tensor, kBoundary, runtime);
    });
    ...
    if (plain_tensor)
    {
        std::vector<double> plain_averages(kFinalChannels, 0.0);
        for (int channel = 0; channel < kFinalChannels; ++channel)
        {
            for (int row = 0; row < plain_tensor->h; ++row)
            {
                for (int col = 0; col < plain_tensor->w; ++col)
                {
                    plain_averages[channel] +=
                        plain_tensor->values[
                            (static_cast<std::size_t>(channel) * plain_tensor->h + row) *
                                plain_tensor->w + col] *
                        kBoundary /
                        static_cast<double>(plain_tensor->h * plain_tensor->w);
                }
            }
        }
        std::vector<double> plain_logits(kClassCount, 0.0);
        for (int output = 0; output < kClassCount; ++output)
        {
            ...
            for (int feature = 0; feature < kFinalChannels; ++feature)
            {
                value += weights.linear_weight[
                         ...
                         plain_averages[feature];
            }
            plain_logits[output] = value;
            result.max_logit_error = std::max(
                result.max_logit_error,
                std::abs(result.logits[output] - value));
        }
        const int plain_prediction = argmax(plain_logits);
        std::cout << "[GPU ResNet50] full_validation plain_pred="
                  << plain_prediction
        ...
        {
            throw std::runtime_error("GPU ResNet50 full validation failed");
        }
    }
    std::cout << "[GPU ResNet50] image=" << image_id
    ...
    std::size_t max_blocks)
{
    ...
    if (max_blocks == 0)
    {
        std::cout << "[GPU ResNet50] generating inference evaluation keys\n";
        runtime.initialize_resnet50_evaluation_keys();
    }
    else
    {
        std::cout << "[GPU ResNet50] initializing shared inference/bootstrap keys\n";
    ...
    std::size_t max_blocks)
{
    if (max_blocks > topology.blocks.size())
    {
        throw std::invalid_argument("ResNet50 preloaded max_blocks exceeds topology");
    }
    GpuCkksRuntime runtime(config);
    if (max_blocks == 0)
    {
    ...
    std::cout << "[GPU ResNet50] preparing device-resident input/model cache\n";
    const auto prepared_result = run_gpu_resnet50_impl(
        image_id, topology, weights, max_blocks, runtime,
        /*enable_validation=*/false, /*measure_gpu_only=*/false,
        /*measure_staged_gpu_only=*/false);
    runtime.synchronize();
    std::cout << "[GPU ResNet50] preparation complete; starting GPU-only pass\n";
    auto measured_result = run_gpu_resnet50_impl(
        image_id, topology, weights, max_blocks, runtime,
    ...
    std::cout << "[GPU ResNet50] preloaded_replay_max_logit_error="
              << replay_logit_error << '\n';
    ...
    std::size_t max_blocks)
{
    if (max_blocks > topology.blocks.size())
    {
        throw std::invalid_argument("ResNet50 staged max_blocks exceeds topology");
    }
    GpuCkksRuntime runtime(config);
    if (max_blocks == 0)
    {
    ...
    const ResNet50Weights &weights)
{
    ...
    GpuResNet50Result result;
    result.image_id = image_id;
    result.completed_blocks = topology.blocks.size();
    result.true_label = load_imagenet_label(image_id);
    const auto image = load_imagenet_image_chw(image_id, kBoundary);
    auto features = timed("head_check.plain_features", [&]() {
        return plain_resnet50_features(image, topology, weights);
    });
    if (features.h != 7 || features.w != 7 || features.c != kFinalChannels)
    {
        throw std::runtime_error("plain ResNet50 head-check feature shape mismatch");
    }
    ...
    for (int channel = 0; channel < kFinalChannels; ++channel)
    {
        for (int row = 0; row < features.h; ++row)
        {
            ...
            {
                plain_averages[channel] +=
                    features.values[(static_cast<std::size_t>(channel) * features.h + row) *
                                    features.w + col] *
    ...
    GpuCkksRuntime runtime(config);
    runtime.initialize_resnet50_evaluation_keys();
    auto encrypted_features = encrypt_multiplexed_chw(
        features.values, features.h, features.w, features.c, /*k=*/16, runtime);
    // A production layer4.2 activation exits polynomial ReLU at Q11. Match
    // that level so this isolated head test has identical arithmetic cost and
    // scale/noise conditions instead of evaluating the head at fresh Q53.
    for (auto &pack : encrypted_features.packs)
    {
        pack = runtime.drop_to_q_count(pack, 11);
    }
    auto pooled = timed("head_check.gpu_global_avgpool", [&]() {
        return global_average_pool(encrypted_features, kBoundary, runtime);
    });
    auto encrypted_logits = timed("head_check.gpu_fc", [&]() {
        return fully_connected(
    ...
    for (int output = 0; output < kClassCount; ++output)
    {
        ...
        result.max_logit_error = std::max(
            result.max_logit_error,
            std::abs(result.logits[output] - plain_logits[output]));
    }
    const int plain_prediction = argmax(plain_logits);
    result.predicted_label = argmax(result.logits);
    std::cout << "[GPU ResNet50] head_check image=" << image_id
              << " plain_pred=" << plain_prediction
              << " gpu_pred=" << result.predicted_label
              << " max_logit_error=" << result.max_logit_error << '\n';
    if (plain_prediction != result.predicted_label ||
        !std::isfinite(result.max_logit_error) || result.max_logit_error > 1.0e-3)
    {
        throw std::runtime_error("GPU ResNet50 head check failed");
    }
    return result;
}
