...
std::string convolution_weight_cache_prefix(
    ...
    int kernel_w)
{
    return "conv_weight:ptr=" + std::to_string(
               reinterpret_cast<std::uintptr_t>(weights.data())) +
           ":in=" + std::to_string(input.h) + 'x' +
           std::to_string(input.w) + 'x' + std::to_string(input.c) +
           ":k=" + std::to_string(input.k) +
           ":out=" + std::to_string(out_channels) +
           ":stride=" + std::to_string(stride) +
           ":kernel=" + std::to_string(kernel_h) + 'x' +
                               ...
                               const GpuCkksRuntime &runtime)
{
    ...
    const int pad_h = kernel_h / 2;
    const int pad_w = kernel_w / 2;
    const auto weight_cache_prefix = convolution_weight_cache_prefix(
        weights, input, out_channels, stride, kernel_h, kernel_w);
        ...
        for (std::size_t group = 0; group < output_groups; ++group)
        {
            ...
            for (int kh = 0; kh < kernel_h; ++kh)
            {
                for (int kw = 0; kw < kernel_w; ++kw)
                {
                    ...
                    const auto cache_key = weight_cache_prefix +
                        ":replicated:group=" + std::to_string(group) +
                        ":kernel=" + std::to_string(kernel_index);
                    auto weighted = runtime.multiply_plain_rescale_cached_lazy(
                        *spatial, cache_key, [&]() {
                            std::vector<double> compact_weight(
                                input.slot_count, 0.0);
                            for (std::size_t replica = 0;
                                 replica < input_replicas; ++replica)
                            {
                                const std::size_t output_channel =
                                    group * input_replicas + replica;
                                if (output_channel >=
                                ...
                                for (int input_channel = 0;
                                     input_channel < input.c; ++input_channel)
                                {
                                    const std::size_t weight_index =
                                        ((output_channel * input.c + input_channel) *
                                             kernel_h +
            ...
            for (std::size_t replica = 0; replica < input_replicas; ++replica)
            {
                ...
                auto channel_sum = runtime.multiply_plain_rescale_cached_lazy(
                    selected,
                    selector_cache_key(
                        output, static_cast<int>(output_channel)), [&]() {
                        std::vector<double> selector(
                            output.slot_count, 0.0);
                        for (int oh = 0; oh < output.h; ++oh)
                        {
                            for (int ow = 0; ow < output.w; ++ow)
                            {
                                selector[output.slot_index(
                                    static_cast<int>(output_channel), oh,
                                    ow)] = 1.0;
                            }
                        }
                        return selector;
                 ...
                 ++input_pack)
            {
                ...
                for (int kh = 0; kh < kernel_h; ++kh)
                {
                    for (int kw = 0; kw < kernel_w; ++kw)
                    {
                        ...
                        const auto cache_key = weight_cache_prefix +
                            ":general:out_pack=" +
                            std::to_string(output_pack) +
                            ":out_channel=" +
                            std::to_string(output_channel) +
                            ":in_pack=" + std::to_string(input_pack) +
                            ":kernel=" + std::to_string(kernel_index);
                        auto weighted =
                            runtime.multiply_plain_rescale_cached_lazy(
                                *spatial, cache_key, [&]() {
                                    std::vector<double> compact_weight(
                                        input.slot_count, 0.0);
                                    for (const int input_channel : input_channels)
