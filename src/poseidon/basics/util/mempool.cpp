...
MemoryPoolHeadMT::MemoryPoolHeadMT(size_t item_byte_count, bool clear_on_destruction)
    : clear_on_destruction_(clear_on_destruction), locked_(false),
      item_byte_count_(item_byte_count), item_count_(MemoryPool::first_alloc_count),
      first_item_(nullptr)
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "invalid allocation size");
    }
    ...
    {
        new_alloc.data_ptr =
            POSEIDON_MALLOC(mul_safe(MemoryPool::first_alloc_count, item_byte_count_));
    }
...
MemoryPoolItem *MemoryPoolHeadMT::get()
{
    ...
    if (old_first == nullptr)
    {
        ...
        else
        {
            ...
            try
            {
                new_alloc.data_ptr = POSEIDON_MALLOC(new_alloc_byte_count);
            }
            catch (const bad_alloc &)
            {
                // Allocation failed; rethrow
...
Pointer<poseidon_byte> MemoryPoolMT::get_for_byte_count(size_t byte_count)
{
    ...
    while (start < end)
    {
        ...
        {
            return Pointer<poseidon_byte>(mid_head);
        }
    }
    reader_lock.unlock();
    ...
    return Pointer<poseidon_byte>(new_head);
}
...
size_t MemoryPoolMT::alloc_byte_count() const
{
    ...
    return accumulate(
        pools_.cbegin(), pools_.cend(), size_t(0),
...
Pointer<poseidon_byte> MemoryPoolST::get_for_byte_count(size_t byte_count)
{
