...
namespace util
{
template <typename... Ts> struct VoidType
{
    using type = void;
};
...
struct IsUInt64 : std::conditional<std::is_integral<T>::value && std::is_unsigned<T>::value &&
...
struct IsUInt64<T, U, Rest...> : std::conditional<IsUInt64<T>::value && IsUInt64<U, Rest...>::value,
                                                  std::true_type, std::false_type>::type
{
};

template <typename T, typename... Rest> constexpr bool is_uint64_v = IsUInt64<T, Rest...>::value;

template <typename T, typename...>
struct IsUInt32 : std::conditional<std::is_integral<T>::value && std::is_unsigned<T>::value &&
...
template <typename T, typename U, typename... Rest>
struct IsUInt32<T, U, Rest...> : std::conditional<IsUInt32<T>::value && IsUInt32<U, Rest...>::value,
                                                  std::true_type, std::false_type>::type
{
};

template <typename T, typename... Rest> constexpr bool is_uint32_v = IsUInt32<T, Rest...>::value;

template <typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
          typename = std::enable_if_t<std::is_integral<S>::value>>
POSEIDON_NODISCARD inline constexpr bool unsigned_lt(T in1, S in2) noexcept
{
...
template <typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
          typename = std::enable_if_t<std::is_integral<S>::value>>
POSEIDON_NODISCARD inline constexpr bool unsigned_leq(T in1, S in2) noexcept
{
    return static_cast<std::uint64_t>(in1) <= static_cast<std::uint64_t>(in2);
}
...
template <typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
          typename = std::enable_if_t<std::is_integral<S>::value>>
POSEIDON_NODISCARD inline constexpr bool unsigned_gt(T in1, S in2) noexcept
{
    return static_cast<std::uint64_t>(in1) > static_cast<std::uint64_t>(in2);
}
...
template <typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
          typename = std::enable_if_t<std::is_integral<S>::value>>
POSEIDON_NODISCARD inline constexpr bool unsigned_geq(T in1, S in2) noexcept
{
    return static_cast<std::uint64_t>(in1) >= static_cast<std::uint64_t>(in2);
}
...
template <typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
          typename = std::enable_if_t<std::is_integral<S>::value>>
POSEIDON_NODISCARD inline constexpr bool unsigned_eq(T in1, S in2) noexcept
{
    return static_cast<std::uint64_t>(in1) == static_cast<std::uint64_t>(in2);
}
...
template <typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
          typename = std::enable_if_t<std::is_integral<S>::value>>
POSEIDON_NODISCARD inline constexpr bool unsigned_neq(T in1, S in2) noexcept
{
    return static_cast<std::uint64_t>(in1) != static_cast<std::uint64_t>(in2);
}

template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
POSEIDON_NODISCARD inline constexpr T mul_safe(T in1) noexcept
{
    return in1;
}

template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
POSEIDON_NODISCARD inline constexpr T mul_safe(T in1, T in2)
{
    POSEIDON_IF_CONSTEXPR(std::is_unsigned<T>::value)
    {
...
#if (POSEIDON_COMPILER == POSEIDON_COMPILER_MSVC) && !defined(POSEIDON_USE_IF_CONSTEXPR)
#pragma warning(push)
...
POSEIDON_NODISCARD inline constexpr T mul_safe(T in1, T in2, Args &&...args)
{
    return mul_safe(mul_safe(in1, in2), mul_safe(std::forward<Args>(args)...));
}

template <typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
POSEIDON_NODISCARD inline constexpr T add_safe(T in1) noexcept
{
    return in1;
}

template <typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
POSEIDON_NODISCARD inline constexpr T add_safe(T in1, T in2)
{
    POSEIDON_IF_CONSTEXPR(std::is_unsigned<T>::value)
    {
...
POSEIDON_NODISCARD inline constexpr T add_safe(T in1, T in2, Args &&...args)
{
    return add_safe(add_safe(in1, in2), add_safe(std::forward<Args>(args)...));
}

template <typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
POSEIDON_NODISCARD inline T sub_safe(T in1, T in2)
{
    POSEIDON_IF_CONSTEXPR(std::is_unsigned<T>::value)
    {
        if (in1 < in2)
...
POSEIDON_NODISCARD inline constexpr bool fits_in(S value POSEIDON_MAYBE_UNUSED) noexcept
{
    bool result = false;

    POSEIDON_IF_CONSTEXPR(std::is_same<T, S>::value)
    {
        // Same type
        result = true;
    }
    else POSEIDON_IF_CONSTEXPR(sizeof(S) <= sizeof(T))
    {
        ...
        POSEIDON_IF_CONSTEXPR(std::is_integral<T>::value && std::is_integral<S>::value)
        {
            ...
            POSEIDON_IF_CONSTEXPR((std::is_unsigned<T>::value && std::is_unsigned<S>::value) ||
                                  (!std::is_unsigned<T>::value && !std::is_unsigned<S>::value))
            {
                // Both either signed or unsigned
                result = true;
            }
            else POSEIDON_IF_CONSTEXPR(std::is_unsigned<T>::value && std::is_signed<S>::value)
            {
                // Converting from signed to at least equally big unsigned type
                result = value >= 0;
            }
        }
        else POSEIDON_IF_CONSTEXPR(std::is_floating_point<T>::value &&
...
template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
POSEIDON_NODISCARD inline constexpr bool sum_fits_in(Args &&...args)
{
    return fits_in<T>(add_safe(std::forward<Args>(args)...));
}

template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
POSEIDON_NODISCARD inline constexpr bool sum_fits_in(T in1, Args &&...args)
{
    return fits_in<T>(add_safe(in1, std::forward<Args>(args)...));
}

template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
POSEIDON_NODISCARD inline constexpr bool product_fits_in(Args &&...args)
{
    return fits_in<T>(mul_safe(std::forward<Args>(args)...));
}

template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
POSEIDON_NODISCARD inline constexpr bool product_fits_in(T in1, Args &&...args)
{
...
template <typename T, typename S, typename = std::enable_if_t<std::is_arithmetic<T>::value>,
          typename = std::enable_if_t<std::is_arithmetic<S>::value>>
POSEIDON_NODISCARD inline T safe_cast(S value)
{
    POSEIDON_IF_CONSTEXPR(!std::is_same<T, S>::value)
    {
        if (!fits_in<T>(value))
...
POSEIDON_NODISCARD inline constexpr int hamming_weight(unsigned char value)
{
...
POSEIDON_NODISCARD inline constexpr T reverse_bits(T operand) noexcept
{
    POSEIDON_IF_CONSTEXPR(is_uint32_v<T>)
    {
    ...
    else POSEIDON_IF_CONSTEXPR(is_uint64_v<T>)
    {
// Temporarily disable UB warnings when `if constexpr` is not available.
#ifndef POSEIDON_USE_IF_CONSTEXPR
#if (POSEIDON_COMPILER == POSEIDON_COMPILER_MSVC)
#pragma warning(push)
#pragma warning(disable : 4293)
#elif (POSEIDON_COMPILER == POSEIDON_COMPILER_GCC)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wshift-count-overflow"
#elif (POSEIDON_COMPILER == POSEIDON_COMPILER_CLANG)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wshift-count-overflow"
#endif
#endif
        return static_cast<T>(reverse_bits(static_cast<std::uint32_t>(operand >> 32))) |
               (static_cast<T>(reverse_bits(static_cast<std::uint32_t>(operand & T(0xFFFFFFFF))))
                << 32);
#ifndef POSEIDON_USE_IF_CONSTEXPR
#if (POSEIDON_COMPILER == POSEIDON_COMPILER_MSVC)
#pragma warning(pop)
...
template <typename T, typename = std::enable_if_t<is_uint32_v<T> || is_uint64_v<T>>>
POSEIDON_NODISCARD inline T reverse_bits(T operand, int bit_count)
{
#ifdef POSEIDON_DEBUG
    if (bit_count < 0 || static_cast<std::size_t>(bit_count) >
...
inline void get_msb_index_generic(unsigned long *result, std::uint64_t value)
{
...
#endif
    static const unsigned long de_bruijn_table64[64] = {
    ...
    *result = de_bruijn_table64[((value - (value >> 1)) * std::uint64_t(0x07EDD5E59A4E28C2)) >> 58];
}

POSEIDON_NODISCARD inline int get_significant_bit_count(std::uint64_t value)
{
    if (value == 0)
    {
        return 0;
    ...
    POSEIDON_MSB_INDEX_UINT64(&result, value);
    return static_cast<int>(result + 1);
}

POSEIDON_NODISCARD inline bool is_hex_char(char hex)
{
    if (hex >= '0' && hex <= '9')
    {
        return true;
...
POSEIDON_NODISCARD inline char nibble_to_upper_hex(int nibble)
{
#ifdef POSEIDON_DEBUG
...
POSEIDON_NODISCARD inline int hex_to_nibble(char hex)
{
    if (hex >= '0' && hex <= '9')
    {
...
POSEIDON_NODISCARD inline int get_hex_string_bit_count(const char *hex_string, int char_count)
{
#ifdef POSEIDON_DEBUG
...
template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
POSEIDON_NODISCARD inline T divide_round_up(T value, T divisor)
{
#ifdef POSEIDON_DEBUG
    if (value < 0)
    {
...
template <typename T> constexpr double epsilon = std::numeric_limits<T>::epsilon();

template <typename T, typename = std::enable_if_t<std::is_floating_point<T>::value>>
POSEIDON_NODISCARD inline bool are_close(T value1, T value2) noexcept
{
...
template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
POSEIDON_NODISCARD inline constexpr bool is_zero(T value) noexcept
{
    return value == T{0};
}
