...
POSEIDON_DOUBLE_HOIST_FUSED_BABY_KEYSWITCH_PLAIN_MAC="${POSEIDON_DOUBLE_HOIST_FUSED_BABY_KEYSWITCH_PLAIN_MAC:-1}"
POSEIDON_DOUBLE_HOIST_FUSED_BABY_BLOCK_SIZE="${POSEIDON_DOUBLE_HOIST_FUSED_BABY_BLOCK_SIZE:-128}"
...
export POSEIDON_DOUBLE_HOIST_FUSED_BABY_KEYSWITCH_PLAIN_MAC
export POSEIDON_DOUBLE_HOIST_FUSED_BABY_BLOCK_SIZE
...
export POSEIDON_EVALMOD_ZERO_COPY_MODDROP
export POSEIDON_EVALMOD_Q_PREFIX_VIEWS
...
echo "[WARN] EvalMod last-use zero-copy ModDrop=${POSEIDON_EVALMOD_ZERO_COPY_MODDROP}."
echo "       Eligible BSGS combine temporaries shorten their logical Q-prefix view in place instead of materializing it with synchronous D2D copies."
echo "       Set POSEIDON_EVALMOD_ZERO_COPY_MODDROP=0 to restore copied Q-prefix materialization for A/B testing."
echo "[WARN] EvalMod shared-operand Q-prefix views=${POSEIDON_EVALMOD_Q_PREFIX_VIEWS}."
echo "       Eligible basis/combine multiplies read a shared high-level ciphertext through a lower-Q const view without materializing the prefix."
echo "       Set POSEIDON_EVALMOD_Q_PREFIX_VIEWS=0 to restore shared-operand ModDrop copies for A/B testing."
"${TEST_BIN}"
