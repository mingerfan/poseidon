...
if [[ "${POSEIDON_BOOTSTRAP_PROFILE}" == "resnet50" ]]; then
    # GPU-compatible form of Trident ResNet50's bootstrap parameters:
    # N=2^16, 2^46 application scale, 2^51 EvalMod scale, K=25,
    # message ratio 32, degree-59 sine approximation and two double-angle
    # iterations. The application's literal 46/51-bit primes cannot be stored
    # in the uint32 GPU residue type, so the verified mixed <=32-bit physical
    # Q chain is used with dynamic rescaling instead.
    ...
    export POSEIDON_BOOTSTRAP_P_COUNT="${POSEIDON_BOOTSTRAP_P_COUNT:-12}"
    ...
    export POSEIDON_BOOTSTRAP_LOG_SCALE="${POSEIDON_BOOTSTRAP_LOG_SCALE:-46}"
    export POSEIDON_BOOTSTRAP_Q0_LEVEL="${POSEIDON_BOOTSTRAP_Q0_LEVEL:-1}"
    ...
    export POSEIDON_BOOTSTRAP_EVALMOD_LOG_SCALE="${POSEIDON_BOOTSTRAP_EVALMOD_LOG_SCALE:-51}"
...
elif [[ "${POSEIDON_BOOTSTRAP_PROFILE}" == "dynamic32" ||
    ...
    echo "Supported profiles: resnet50, dynamic32, dual30, legacy" >&2
    exit 2
fi
