...
NCU_KERNEL_NAME="${NCU_KERNEL_NAME:-regex:.*forward_ntt_fused_stage_kernel.*}"
NCU_RELIN_OUTPUT="${NCU_RELIN_OUTPUT:-${BUILD_DIR}/ncu_relinearize_fused3_N${POSEIDON_NSYS_RELIN_DEGREE}_q${POSEIDON_NSYS_RELIN_Q_COUNT}_p${POSEIDON_NSYS_RELIN_P_COUNT}}"
...
if [[ "${RUN_NCU_RELINEARIZE}" != "0" ]]; then
    ...
    POSEIDON_NTT_ALGO="${POSEIDON_NTT_ALGO:-fused}" \
