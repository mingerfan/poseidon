"""Offline audit of frozen v20 paid construction cases; never calls a provider."""
import argparse
import hashlib
import json
...
from object_arithmetic_exercises import CATALOG,EXERCISES,descriptor
...
def audit(batch):
    ...
    require(report.get('object_arithmetic') is True,'Not v20 batch')
    require(report.get('object_arithmetic_exercises')==dict(schema=1,
    ...
    for row in report['cases']:
        ...
        require(exercise['id']==name and set(exercise['influence'])==set(EXERCISES[name]['required_features']),
                'Missing executed/influential object arithmetic')
...
def main():
    ...
    output=Path(tempfile.mkdtemp(prefix='object-arithmetic-agent-audit-',dir=RESULTS))
