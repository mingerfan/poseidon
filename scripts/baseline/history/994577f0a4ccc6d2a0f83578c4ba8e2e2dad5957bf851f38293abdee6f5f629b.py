...
OPS = frozenset(("add", "multiply", "subtract", "negate", "square", "power", "linear", "flatten", "rotate", "batch_norm", "reshape")) | SPATIAL_OPS
...
def evaluate_reference(data, logical_input):
    ...
    for node in data["nodes"]:
        ...
        if op in ("add", "multiply", "subtract"):
            ...
            from logical_reshape import reshape_shape
            result=nested(a,reshape_shape(array_shape(values[node["inputs"][0]]),node["shape"]))
        elif op == "flatten":
            result = a
        elif op == "rotate":
        ...
        if op not in SPATIAL_OPS | {"linear", "flatten", "reshape"} and len(array_shape(values[node["inputs"][0]])) > 1:
            from spatial_ops import nested
