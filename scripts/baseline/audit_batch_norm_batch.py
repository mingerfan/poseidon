"""Offline audit of the fixed six-model BatchNorm paid cohort."""
import argparse
import hashlib
import json
...
MANIFEST=Path(__file__).with_name('cases')/'batch-norm-agent-6-manifest.json'
...
def audit(batch):
    ...
    require(report['custom_manifest']['sha256']==manifest_hash,'Frozen BN cohort changed')
    ...
    for row in report['cases']:
        ...
        require(semantics['operators'].get('batch_norm')==1,'Missing output-reachable BN model')
    ...
    return dict(schema=1,status='covered' if len(rows)==6 else 'incomplete',batch=str(root),batch_sha256=digest,
        manifest_sha256=manifest_hash,planned=6,passed=len(rows),failed=failed,cases=rows,
...
def main():
    ...
    out=Path(tempfile.mkdtemp(prefix='batch-norm-agent-audit-',dir=RESULTS))
