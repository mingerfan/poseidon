"""Opt-in actual BatchNorm FHE evidence; never invokes a provider."""
import hashlib
import json
import os
...
@unittest.skipUnless(os.environ.get('POSEIDON_BATCH_NORM_GOLDENS'),'requires saved real BatchNorm goldens')
class BatchNormEvidenceTests(unittest.TestCase):
    def test_exact_models_sources_security_ciphertexts_and_counterexamples(self):
        ...
        from run_batch_norm_goldens import PLANS
        from test_batch_norm import descriptor,BASE
        ...
        batch,_=metadata(Path(os.environ['POSEIDON_BATCH_NORM_GOLDENS']),RESULTS)
        ...
        for row,(name,source_name,wrong) in zip(batch['cases'],PLANS):
            with self.subTest(name=source_name):
                ...
                self.assertEqual(payload['candidate']['hecate_source'],(BASE/'golden_cases/batch_norm'/(source_name+'.py')).read_text())
