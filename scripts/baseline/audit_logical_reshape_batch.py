"""Offline audit of the fixed three-model logical reshape paid cohort."""
import argparse
import hashlib
import json
...
MANIFEST=Path(__file__).with_name('cases')/'logical-reshape-agent-3-manifest.json'
...
def audit(batch):
    ...
    require(report['custom_manifest']['sha256']==manifest_hash,'Frozen reshape cohort changed')
    ...
    for row in report['cases']:
        ...
        require(semantics['operators'].get('batch_norm')==1 and semantics['operators'].get('reshape')==1,
                'Missing output-reachable reshape/BN composition')
        ...
        with np.load(run/'arrays.npz',allow_pickle=False) as arrays:
            expected=[evaluate_reference(row['descriptor'],x.reshape(row['descriptor']['input_shape']).tolist())
                      for x in arrays['inputs']]
            require(np.allclose(arrays['reference'],expected,atol=1e-12,rtol=1e-12),'Independent BN reference mismatch')
    ...
    return dict(schema=1,status='covered' if len(rows)==3 else 'incomplete',batch=str(root),batch_sha256=digest,
        manifest_sha256=manifest_hash,planned=3,passed=len(rows),failed=failed,cases=rows,
...
def main():
    ...
    out=Path(tempfile.mkdtemp(prefix='logical-reshape-agent-audit-',dir=RESULTS))
