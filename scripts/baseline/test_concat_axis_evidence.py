...
@unittest.skipUnless(os.environ.get('POSEIDON_CONCAT_AXIS_GOLDENS'),'requires saved real Concat goldens')
class ConcatAxisEvidenceTests(unittest.TestCase):
    def test_exact_models_sources_security_ciphertexts_and_counterexamples(self):
        ...
        from run_concat_axis_goldens import PLANS
        ...
        batch,_=metadata(Path(os.environ['POSEIDON_CONCAT_AXIS_GOLDENS']),RESULTS)
