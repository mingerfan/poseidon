...
OP_SEMANTICS = {
    ...
    'concat':'concat', 'reshape':'shape_layout', 'batch_norm':'batch_norm', 'square':'polynomial', 'power':'polynomial', 'linear':'linear', 'flatten':'flatten',
...
def analyze_graph(data):
    ...
    for node in data['nodes']:
        ...
        if op == 'reshape':
            features.add('reshape.target.'+dimensions(node['shape']))
            features.add('reshape.c_order_no_permutation')
        if op == 'concat':
            features.add('concat.axis.'+str(node['axis']))
            features.add('concat.arity.'+str(len(node['inputs'])))
        if op == 'batch_norm':
            features.add('batch_norm.fixed_running_statistics')
            features.add('batch_norm.eps.'+str(node['eps']))
            features.add('batch_norm.affine.'+str(node['weight'] is not None or node['bias'] is not None).lower())
        if op == 'power':
