...
build="$POSEIDON_WORK_ROOT/build-dacapo/hecate-18.1.2-nix"
...
test "$CC" = "$NIX_CC/bin/clang"
test "$CXX" = "$NIX_CC/bin/clang++"
test "$(timeout -k 3s 20s llvm-config --version)" = 18.1.2
timeout -k 3s 20s mlir-opt --version | grep -F '18.1.2'
