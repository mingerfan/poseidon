...
OPS = frozenset(("add", "multiply", "subtract", "negate", "square", "power", "linear", "flatten", "rotate", "batch_norm", "reshape", "concat")) | SPATIAL_OPS
NAME = re.compile(r"[A-Za-z][A-Za-z0-9_]{0,47}\Z")
CASE_ID = re.compile(r"[A-Za-z][A-Za-z0-9_-]{0,63}\Z")
LINEAR_WIDTH_LIMIT = 8


def array_shape(value, depth=0, *, max_elements=128):
    ...
    if type(value) in (int, float):
        ...
        return ()
    require(type(value) is list and 1 <= len(value) <= max_elements, "Invalid public array")
    children = [array_shape(x, depth + 1, max_elements=max_elements) for x in value]
    require(all(x == children[0] for x in children), "Ragged public arrays are forbidden")
    shape = (len(value), *children[0])
    require(math.prod(shape) <= max_elements, "Public array element limit")
    return shape


def input_specs(data):
    """Ordered user names/shapes; never infer argument order from a JSON object."""
    if data.get("schema") in (2, 4, 5):
        return [{"name": "x", "shape": data["input_shape"]}]
    return data["inputs"]


def validate_graph(data):
    if type(data) is dict and type(data.get('schema')) is int and data['schema'] == 5:
        from packed_model import validate
        return validate(data)
    if type(data) is dict and type(data.get('schema')) is int and data['schema'] == 4:
        from chunked_model import validate
        return validate(data)
    ...
    for node in nodes:
        ...
        inputs = node["inputs"]
        require(type(inputs) is list and (1 <= len(inputs) <= 8 if op == "concat" else
                len(inputs) == (2 if op in ("add", "multiply", "subtract") else 1)),
        ...
        if op in ("add", "multiply", "subtract"):
            ...
            from concat_ops import geometry as concat_geometry
            require(all(a[0] == "cipher" for a in args), "Concat requires ciphertext tensors")
            out, _ = concat_geometry([a[1] for a in args], node["axis"])
            ...
            require(left[0]=="cipher" and type(node["shape"]) is list,"Reshape requires cipher and explicit target")
            result=("cipher",reshape_shape(left[1],node["shape"]),left[2])
        elif op == "flatten":
...
def build_graph_model(data):
        ...
        def evaluate(self, args):
            ...
            for node in self.definition["nodes"]:
                ...
                if op == "add":
                    ...
                    result = torch.reshape(args[0],tuple(node["shape"]))
                elif op == "permute":
                    result = args[0].permute(tuple(node['dims']))
                elif op == "transpose":
                    result = torch.transpose(args[0],node['dim0'],node['dim1'])
                elif op == "concat":
                    result = torch.cat(args, node["axis"])
                elif op == "flatten":
    ...
    shape = list(data["input_shape"]) if data["schema"] in (2, 4, 5) else {"inputs": copy.deepcopy(data["inputs"])}
...
def evaluate_reference(data, logical_input):
    ...
    supplied = {"x": logical_input} if data["schema"] in (2, 4, 5) else logical_input
    specs = input_specs(data)
    require(type(supplied) is dict and set(supplied) == {s["name"] for s in specs}, "Reference input name mismatch")
    for spec in specs:
        require(array_shape(supplied[spec["name"]], max_elements=4096 if data['schema']==5 else 128) == tuple(spec["shape"]), "Reference input shape mismatch")
    ...
    for node in data["nodes"]:
        ...
        if op in ("add", "multiply", "subtract"):
            ...
            if data['schema']==5:
                left_shape=array_shape(values[node['inputs'][0]],max_elements=4096)
                right_value=values[node['inputs'][1]]
                right_shape=array_shape(right_value,max_elements=4096)
                # Independent indexed broadcasting, not Torch or lowering masks.
                b=[]
                for index in range(len(a)):
                    coordinates=[(index//math.prod(left_shape[k+1:]))%left_shape[k] for k in range(len(left_shape))]
                    value=right_value
                    for dimension,coordinate in zip(right_shape,coordinates[len(left_shape)-len(right_shape):]):
                        value=value[0 if dimension==1 else coordinate]
            ...
            if data['schema']==5:
                from packed_spatial import reference as spatial_reference
            else:
            ...
            limit=4096 if data['schema']==5 else 128
            kernel = array_shape(weight,max_elements=limit)[2:] if weight is not None else node["kernel"]
            result = spatial_reference(op, value, array_shape(value,max_elements=limit), weight, bias, kernel,
                                       node["stride"], node["padding"], node.get("count_include_pad", True),
                                       dilation=node.get("dilation"), groups=node.get("groups", 1))
        elif op == "batch_norm":
            shape=array_shape(values[node["inputs"][0]],max_elements=4096 if data['schema']==5 else 128)
            ...
            shape=array_shape(values[node['inputs'][0]],max_elements=4096 if data['schema']==5 else 128)
            width=shape[-1];rows=values[node['weight']]
            result=[]
            for begin in range(0,len(a),width):
                for i,row in enumerate(rows):
                    value=math.fsum(float(w)*a[begin+j] for j,w in enumerate(row))
                    result.append(value+(0. if node['bias'] is None else float(values[node['bias']][i])))
            if len(shape)>1:
                from spatial_ops import nested
                result=nested(result,(*shape[:-1],len(rows)))
        elif op == "negate":
            result = [-u for u in a]
            ...
            from tensor_permutation import reference as permutation_reference,transpose_axes
            value=values[node['inputs'][0]];shape=array_shape(value,max_elements=4096)
            dims=node['dims'] if op=='permute' else transpose_axes(shape,node['dim0'],node['dim1'])
            result=permutation_reference(value,shape,dims)
        elif op == "reshape":
            from spatial_ops import nested
            from logical_reshape import reshape_shape
            result=nested(a,reshape_shape(array_shape(values[node["inputs"][0]],max_elements=4096 if data['schema']==5 else 128),
                node['shape'],max_elements=256 if data['schema']==5 else 8))
        elif op == "concat":
            from concat_ops import reference as concat_reference
            result=concat_reference([values[n] for n in node["inputs"]],node["axis"])
        elif op == "flatten":
            result = a
        elif op == "rotate":
        ...
        if op not in SPATIAL_OPS | {"linear", "flatten", "reshape", "concat", "permute", "transpose"} and len(array_shape(values[node["inputs"][0]], max_elements=4096 if data['schema']==5 else 128)) > 1:
            from spatial_ops import nested
            result = nested(result, array_shape(values[node["inputs"][0]],max_elements=4096 if data['schema']==5 else 128))
