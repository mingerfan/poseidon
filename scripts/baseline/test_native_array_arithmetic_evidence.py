"""Re-audit saved native array arithmetic; no API or new encrypted execution."""
import ast
import json
import os
...
@unittest.skipUnless(os.environ.get('POSEIDON_NATIVE_ARRAY_ARITHMETIC_GOLDENS'),'requires completed real FHE batch')
class NativeArrayArithmeticEvidenceTests(unittest.TestCase):
    def test_real_compiler_artifacts_order_and_independent_reference(self):
        import numpy as np
        ...
        from native_function_rules import ARITHMETIC_TASK
        from run_native_array_arithmetic_goldens import CASES
        from seal_cpu_golden import compare
        root=Path(os.environ['POSEIDON_NATIVE_ARRAY_ARITHMETIC_GOLDENS'])
        ...
        for name,row in zip(CASES,batch['cases']):
            with self.subTest(name=name):
                ...
                self.assertEqual((params['seal_version'],params['security_check'],params['polynomial_degree']),
                                 ('4.0.0','tc128',32768))
                ...
                self.assertEqual(request['task'],ARITHMETIC_TASK)
                ...
                tree=ast.parse(candidate['hecate_source']);names={f.name for f in tree.body}
                ...
                independent=inputs**2*.5+.125 if name=='square' else inputs*1.5+.375
                ...
                if row['counterexample']:
                    ...
                    np.testing.assert_allclose(actual,inputs*1.75+.375,atol=1e-5,rtol=1e-4)
