...
timeout -k 3s 90s bash scripts/baseline/nix_portable.sh \
  ...
  --argstr platform "$POSEIDON_PLATFORM" --file src/poseidon/tools/dacapo/dacapo-dependencies.nix metadata \
  | tee "$report.json"
timeout -k 3s 180s bash scripts/baseline/nix_portable.sh \
  ...
  --argstr platform "$POSEIDON_PLATFORM" --file src/poseidon/tools/dacapo/dacapo-shell.nix 2>&1 \
  | tee "$report.log"
