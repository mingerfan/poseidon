"""Offline audit of frozen v21 paid construction cases; never calls a provider."""
import argparse
import hashlib
import json
...
from scalar_conversion_exercises import CATALOG,EXERCISES,descriptor
...
def audit(batch):
    ...
    require(report.get('scalar_conversion') is True,'Not v21 batch')
    require(report.get('scalar_conversion_exercises')==dict(schema=1,
    ...
    for row in report['cases']:
        ...
        require(exercise['id']==name and set(exercise['influence'])==set(EXERCISES[name]['required_features']),
                'Missing executed/influential scalar extraction')
...
def main():
    ...
    output=Path(tempfile.mkdtemp(prefix='scalar-conversion-agent-audit-',dir=RESULTS))
