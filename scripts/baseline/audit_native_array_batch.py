"""Recheck saved array-construction evidence; no API or FHE execution.

Live mode is the default. Manual mode also requires the real wrong-transpose
...
from native_array_exercises import CATALOG,EXERCISES,descriptor,verify_trace_coverage
...
def audit_run(run,name,*,live,wrong=False):
    ...
    require(canonical(checked)==canonical(attempt['static_check']),'Static array coverage changed')
    hashes = attempt['artifact_hashes']
    require({'native-array-events.json','native-call-events.json','native-function-plan.json',
    ...
    observed,_ = metadata(out/'native-array-events.json',out)
    coverage = verify_trace_coverage(checked['construction_exercise'],observed)
    require(coverage==attempt['native_array_coverage'],'Actual array coverage mismatch')
    ...
    if wrong: require(attempt['failure_layer']=='numerical_comparison','Wrong transpose not rejected by numbers')
...
def audit(batch,*,manual=False):
    ...
    if manual:
        require(report['status']=='passed' and report.get('generator')=='manual_golden'
                and report.get('agent_calls')==0,'Not a completed manual array cohort')
        require([r['case'] for r in report['cases']]==['exercise-'+n for n in names]+['exercise-na-transpose']
                and [r['counterexample'] for r in report['cases']]==[False]*10+[True],'Wrong manual cohort')
        require(all(r['matched_expected'] for r in report['cases']),'Manual expectation failed')
        counterexample = audit_run(report['cases'][-1]['run'],'na-transpose',live=False,wrong=True)
    else:
        require(report['status'] in ('passed','completed_with_failures') and report.get('native_arrays') is True,
                'Not a terminal live array cohort')
        require(report.get('native_array_exercises')==dict(schema=1,catalog_sha256=hashlib.sha256(CATALOG.read_bytes()).hexdigest()),
                'Array catalog mismatch')
    ...
    matrix = [dict(feature=f,cases=[p['id'] for p in passed if f in p['exercise']['required']],
                   evidence_kind='trace_structural_only' if f=='return.empty' else 'finite_cell_result_influence') for f in features]
...
def main():
    ...
    output=Path(tempfile.mkdtemp(prefix='native-array-exercise-audit-',dir=RESULTS))
