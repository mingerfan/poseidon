...
BUILD_DIR="${BUILD_DIR:-${REPO_ROOT}/build-mgpu-nccl-bench}"
...
echo "=== Configure NCCL benchmark ==="
"${CMAKE_BIN}" "${CMAKE_ARGS[@]}"

echo "=== Build NCCL benchmark ==="
"${CMAKE_BIN}" --build "${BUILD_DIR}" --target poseidon_mgpu_nccl_transfer_bench -j"${JOBS:-2}"

BENCH_BIN="${BUILD_DIR}/bin/poseidon_mgpu_nccl_transfer_bench"
if [[ ! -x "${BENCH_BIN}" ]]; then
    echo "Benchmark binary was not produced: ${BENCH_BIN}" >&2
    exit 1
fi

echo "=== Run NCCL benchmark ==="
